...
#     # Set up the subscription info and region for the Speech Service:
#     speech_key = 'bb855bc73cd6416b9902fd6c450e2cda'
...
#     # Create a speech recognizer using the file as audio input.
#     speech_recognizer = speechsdk.SpeechRecognizer(speech_config=speech_config, audio_config=audio_config)

#     # Initialize a list to hold the results
#     recognized_texts = []

#     # Define callbacks for the recognition events
#     def recognizing_cb(event):
#         print(f"RECOGNIZING: {event.result.text}")

#     def recognized_cb(event):
#         if event.result.reason == speechsdk.ResultReason.RecognizedSpeech:
#             recognized_texts.append(event.result.json)
...
#             print(f"CANCELED: {event.error_details}")
...
#     input("Press Enter to stop the recognition...")
#     speech_recognizer.stop_continuous_recognition()

#     return recognized_texts


# # Open a text file and write recognized_texts into it
# with open('recognized_texts.json', 'w') as file:
#     json.dump(updated_segments, file)
